...
#
#     objs = []
#     unique_classes = np.unique(segmentation)  # 使用np.unique获取类别
...
def get_pixocial_19_dicts(img_dir, seg_dir):
...
PIXOCIAL_19_CLASSES = [c["name"] for c in PIXOCIAL_19_CATEGORIES]
PIXOCIAL_19_COLORS = [c["color"] for c in PIXOCIAL_19_CATEGORIES]

_root = os.getenv("DETECTRON2_DATASETS", "datasets")

# 注册训练集和验证集
for d in ["train", "val"]:
    img_root = f"{_root}/pixocial_19/{d}_images"
    gt_root = f"{_root}/pixocial_19/{d}_segmentations"
    DatasetCatalog.register(
      f"pixocial_19_{d}",
      lambda img_dir=img_root, seg_dir=gt_root: get_pixocial_19_dicts(img_dir, seg_dir)
    )
    MetadataCatalog.get(f"pixocial_19_{d}").set(stuff_classes=PIXOCIAL_19_CLASSES)
    MetadataCatalog.get(f"pixocial_19_{d}").set(stuff_colors=PIXOCIAL_19_COLORS)
    MetadataCatalog.get(f"pixocial_19_{d}").set(evaluator_type="sem_seg")
    MetadataCatalog.get(f"pixocial_19_{d}").set(ignore_label=255)
    MetadataCatalog.get(f"pixocial_19_{d}").set(image_root=img_root)
    MetadataCatalog.get(f"pixocial_19_{d}").set(sem_seg_root=gt_root)
